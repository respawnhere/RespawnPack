...
- **Safety-check invariants** — a PASS after filters/reads must have `checked > 0`; returned outcomes must equal the rollup of the checks callers consume; unreadable/skipped-by-limitation inputs prevent PASS; absent, malformed, partial, unsupported, undecided and not-applicable stay distinct; a seeded `<set X>` template is never read as a value; "the machinery works" and "the contract is decided" are reported as separate verdicts and the exit stays the worse of them; schema-valid and runtime-accepted shapes agree in both directions.
- **Ownership and messaging** — matching bytes are not installer provenance; founder-owned values need explicit migration authorization. Rerun/dry-run/upgrade messages must inspect the effective active state, including custom paths.
- **Release scope** — pre-push checks derive work from every ref on Git's stdin, not HEAD; merge commits, the target remote, ignored durable artifacts, and command failures cannot disappear from the verdict.
