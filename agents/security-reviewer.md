...
- **Filesystem authority** — every configured registry, scan root, source, archive, and output path remains inside the project after `realpath`; a missing output is judged through its nearest existing ancestor before creation. Lexical containment is not enough because a root symlink can escape. Custom authority paths must also participate in freshness.
- **Coverage bypasses** — inferred exclusions, broad history paths, unrelated suppression markers, oversized files, unreadable files, and symlinks must not turn missing coverage into PASS. Test the nearest bypass, not only the ideal malicious input.
