...
function evaluate({ usedPercent, source, thresholds = DEFAULT_THRESHOLDS, record }) {
  ...
  /*
   * ⛔ THE FINAL THRESHOLD ON ANYTHING BUT A FULLY MEASURED NUMBER IS FLAGGED, NOT SUPPRESSED AND NOT
   * TRUSTED. Claude's transcript format is explicitly internal, a byte proxy is our arithmetic rather
   * than the host's, and a documented count divided by a window WE configured is a ratio only half of
   * which the host published. Stopping the session on any of them without saying so would present an
   * estimate as a reading.
   *
   * ⛔ THE TEST IS `!== HIGH`, AND IT STAYS THAT WAY AS TIERS ARE ADDED. It is not a ranking and not a
   * list of the weak sources — a new tier is caught by this line on the day it is declared, without
   * anyone remembering to add it. MEDIUM is not a partial exemption: it is confirmed exactly as
   * strictly as PROXY.
   */
