...
## Orchestration
A sweep fans out one subagent per page via the Agent tool — **max 4 concurrent**, each getting this skill, the contract path, and the app URL — and the dispatching agent synthesizes: reports naming the same shell element or route pattern merge into one finding (a broken nav is one finding, not one per page). A single page runs inline. A `changed` run resolves the current diff to routes via the project's routing convention (file path under the route source, or a route-table lookup); if the mapping is ambiguous, widen to a sweep rather than guess.
