...
- **Confirm the condition.** The spine must be un-spined *and* the codebase real: canonical docs still templates (unfilled `<PLACEHOLDER>`s in `PRODUCT.md`/`FEATURES-PAGES.md`/`DECISIONS.md`), or `respawnpack doctor` reporting the `onboarding` row INCOMPLETE with `codeTruth` among its undecided contracts — **and** substantial pre-existing code (a populated `routeSource`, real `opsTargets`, a source tree). ⛔ `codeTruth` is UNDECIDED when the key is ABSENT: installs before the placeholder was removed carry the literal `<set CODE_TRUTH: …>`, which the kernel classifies INVALID and which is never read as a value. If the spine is already filled, stop and say so: this is a first-run op, and the work belongs in `/loadout`. If the repo is genuinely greenfield, hand back to `/respawn`'s adoption interview.
...
- **Read the target shapes.** Load `respawnpack.config.json` (`routeSource`, `opsTargets` — already installer-detected; `routeSource` may legitimately be ABSENT where the installer detected none) and skim the empty spine templates so the drafts match their canonical shapes.
...
- **WRITE-ONCE** — no section enters `docs/*.md` without a human yes; `codeTruth`/`routeSource` and `DECISIONS` edits are proposed, never auto-written, and a not-applicable is proposed with its reason rather than left silently undecided. <!-- invariant -->
