---
name: secure
description: The security-audit role — a defense-in-depth pass over a change or the repo: OWASP code vulnerabilities, dependency/supply-chain CVEs, and managed-infra advisories. Fans out lenses via the Agent tool and adversarially verifies each finding. Delegates secrets to /secrets-audit.
...
Spawn independent lenses via the Agent tool (a Workflow for a large surface), weighted by Step 0's risk surfaces:
...
- **Sensitive-data exposure** — code paths that reach a log / response / error sink with PII or secrets; missing transport security; over-broad CORS. (The redaction *config* itself is `/secrets-audit`'s domain — here we flag the leaking code path.)
- **LLM / AI surfaces (OWASP LLM Top 10)** — *only when the target calls a model or runs an agent.* Prompt-injection paths (direct, and indirect via retrieved documents or tool output); insecure handling of model output (rendered to a page, executed as code, or run as a query without validation); excessive agency (tool or permission grants beyond what the feature needs); sensitive-data leakage into prompts or logs; and model-endpoint cost abuse (unbounded or unauthenticated calls to a paid endpoint). No AI surface → skip this lens entirely; don't invent one.

Keep concurrency moderate (single-digit). **Adversarially verify** each finding — a skeptic pass that tries to *refute* it (default "not exploitable" unless the evidence holds); drop or downgrade what doesn't survive; force any finding you can't verify to low confidence. If the Workflow tool isn't available in this environment, run the same lenses as sequential Agent calls and verify findings before reporting.
