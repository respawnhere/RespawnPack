...
## 7. Lead with the bottom line

A reader who has to read to the end to learn whether something worked has already spent the attention this rule exists to protect.

- Open every response with the outcome or the answer, in one or two sentences: what happened, what the answer is, or what changed. The evidence and the reasoning follow it; they do not precede it.
- A report or a hand-off opens with a BLUF line: the state of things in one sentence, so a reader with ten seconds still gets the one fact that matters before they decide whether to read on.
- A failure or an unverified step is never below the fold. If something did not work, or was not checked, that goes in the first sentence, not the appendix a reader may never reach.
- Good: "Tests pass; the off-by-one is fixed in `parser.js:88`." then the root cause and the regression test. Bad: "I traced through the parser, checked the tokenizer, found the loop bound was wrong in a couple of places, fixed the one that mattered, and after a few runs the tests pass now." Both report the same fix; only the first tells the reader what they need before asking them to read a paragraph to find it.

This is [`writing-standards.md`](writing-standards.md) principle 3 (lead with the point) applied to session conduct: not only how a sentence is built, but where in a response the point has to land.
