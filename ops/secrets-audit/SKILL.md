...
Scan tracked files + history for credential patterns (the same patterns as the [secret-scan hook](../../hooks/secret-scan.js) — private keys, `sk_live_`, `AKIA…`, GH/Slack/Google tokens, generic `key=…` assignments). The hook is the push-time backstop; this is the audit. Report HIGH-severity hits with file:line — **but reference, never echo, the value.**
